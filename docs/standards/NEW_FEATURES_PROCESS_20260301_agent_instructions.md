...
Use the `docs/standards/NEW_FEATURES_PROCESS_20260301.md` plan to coordinate across these five sequential AI agent sessions. 
...
2. Initialize `tmp/new_feature_state-<feature_name>.md` to keep track of decisions, out-of-scope follow-ups, API contracts, and architectural updates. 
...
5. Define security, input sanitization, error handling, and privacy limits (e.g. data retention, anonymization). Log all decisions in `tmp/new_feature_state-<feature_name>.md`.
```
...
1. Read `README.md`, `docs/standards/NEW_FEATURES_PROCESS_20260301.md`, and `tmp/new_feature_state-<feature_name>.md`. 
...
5. Update `tmp/new_feature_state-<feature_name>.md` with the files modified and functionality added. 
```
...
5. Confirm that the API contracts are fully respected, maintaining modularity. Document constraints or issues in `tmp/new_feature_state-<feature_name>.md`.
```
...
4. Check for dead/unused code, ensure all imports are ONLY at the top of files, and ensure no unintended files are present (check tracking/commits). Document any omitted features or non-critical bugs found in `tmp/new_feature_state-<feature_name>.md`.
```
...
1. Read `README.md`, `docs/standards/NEW_FEATURES_PROCESS_20260301.md`, and `tmp/new_feature_state-<feature_name>.md`.
2. Draft a markdown summary file highlighting the omitted follow-up steps. In that summary, suggest further optimizations such as new frameworks, libraries, packages or tools; performance tunings, Python syntax improvements; enhanced security techniques, or improved user experience.
3. Review the whole implementation to ensure a clean codebase—remove `tmp/new_feature_state-<feature_name>.md` and any other ad-hoc temp tools. 
