...
def test_daily_values_text_schema_and_insert_roundtrip(tmp_path: Path):
    ...
    cur.execute(
        ...
        "(id INTEGER PRIMARY KEY AUTOINCREMENT, cik TEXT NOT NULL)"
    )
    cur.execute(
        "CREATE TABLE dates "
    ...
    cur.execute(
        ...
        "  value_text TEXT,\n"
        "  CONSTRAINT uq_daily_values_text_entity_date_value"
    ...
    cur.execute(
        "INSERT INTO daily_values_text"
        "(entity_id, date_id, value_name_id, value_text) VALUES (?,?,?,?)",
    ...
    # Verify roundtrip
    got = cur.execute(
        "SELECT value_text FROM daily_values_text"
        " WHERE entity_id=? AND date_id=? AND value_name_id=?",
